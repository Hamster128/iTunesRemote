...
executeScript = function(scriptName, cb) {
...
executeScriptRes = function(scriptName, cb) {
...
doNextInQueue = function() {
  ...
  if(job.func == 'play') {
    ...
    executeScriptRes('currentTrack.js', function(err, stdout, stderr) {
      ...
      if(job.cb)
      {
        ...
        try {
          ...
          console.log('itunes.currentTrack error parsing stdout=['+stdout+'] str=['+str+'] stderr=['+stderr+'] '+e);
          job.cb(null);
        }
      }
    ...
    executeScriptRes('playlistTracks.js '+job.id.id_low+' '+job.id.id_high+' '+job.id.skip+' '+job.id.mode+' '+job.id.sortOrder, function(err, stdout, stderr) {
      if(err)
    ...
    executeScript('playTrackInList.js '+job.id.id_low+' '+job.id.id_high+' '+job.id.idx+' '+job.id.sortOrder, function(err, stdout, stderr) {
      if(err)
    ...
    executeScript('setRating.js '+job.r.id_low+' '+job.r.id_high+' '+job.r.rating, function(err, stdout, stderr) {
      if(err)
    ...
    executeScript('removeTrackFromList.js '+job.msg.track.id_low+' '+job.msg.track.id_high+' '+job.msg.id_low+' '+job.msg.id_high, function(err, stdout, stderr) {
      if(err)
    ...
    executeScript('addTrackToList.js '+job.msg.track.id_low+' '+job.msg.track.id_high+' '+job.msg.id_low+' '+job.msg.id_high, function(err, stdout, stderr) {
      if(err)
    ...
    executeScriptRes(execCmd+'artistAlbums.js '+job.id.id_low+' '+job.id.id_high, function(err, stdout, stderr) {
      if(err)
